...
- [x] 3.4 PortForward struct and pfPIDFile helpers
- [x] 3.5 StartPortForward(kubeconfigPath, namespace, service, localPort, remotePort)
...
- [x] 3.9 FindRunningPod(ctx, cs, namespace, pattern) (string, error)
- [x] 3.10 RunPortForwardDaemon(kubeconfigPath, namespace, service, localPort, remotePort) error
...
- [x] 4.5 TestListPortForwards_ParsesPIDFiles
...
- [x] 5.3 hf kube port-forward start <service> <localPort:remotePort>
- [x] 5.4 hf kube port-forward stop <service>
- [x] 5.5 hf kube port-forward status
- [x] 5.6 hf kube curl <service> <path>
- [x] 5.7 hf kube debug <deployment>
- [x] 5.8 hf kube _pf-daemon (hidden, used by StartPortForward)

## 6. cmd/kube_test.go
- [x] 6.1 TestKubeContext_MissingKubeconfig
- [x] 6.2 TestKubePortForwardStatus_Empty
- [x] 6.3 TestKubePFStartCmd_InvalidPorts

## 7. cmd/logs.go
- [x] 7.1 logsCmd with --namespace/-n and --kubeconfig flags
- [x] 7.2 hf logs <pattern>
- [x] 7.3 hf logs adapter <pattern>
...
- [x] (d) Live cluster verification → verification_proof/*.txt
  - hf kube context: prints "hyperfleet-dev-amarin-eu1" ✓
  - hf kube port-forward status: shows empty table ✓
  - hf logs adapter: connects to cluster (401 Unauthorized — kubeconfig not uploaded) ✓
